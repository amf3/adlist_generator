...
class ConfigurationWriter:
    ...
    def __init__(
        ...
        self.local_data = [item for item in finalized_data if item[3] == "local"]
    ...
    def write_local_records_conf(self, filepath: str) -> None:
        ...
        record_map: dict[str, dict] = {item[0]: item[2] for item in self.local_data}
        ...
        try:
            with open(filepath, "w") as f:
                f.write(
                    "# Local authority records — generated by adlist_generator. Do not edit manually.\n\n"
                )
                    ...
                    for domain, records in sorted(forward_records.items()):
                        if domain == zone_apex or domain.endswith(f".{zone_apex}"):
                            for rec_type, value in records.items():
                                f.write(
                                    f'local-data: "{domain}. IN {rec_type} {value}"\n'
                                )
                    f.write("\n")
    ...
    def write_compiled_adblock_conf(self, filepath: str) -> None:
        ...
        try:
            with open(filepath, "w") as f:
                f.write(
                    "# Adblock zones — generated by adlist_generator. Do not edit manually.\n\n"
                )
            ...
            with open(filepath, "w") as f:
                ...
                for domain, _policy, records, source in self.local_data:
                    ...
                    if records:
                        ...
                        f.write(
                            f"{reversed_notation} -> local-data [local] {{{record_str}}}\n"
                        )
